 ...
 * @param buff A buffer with at least max(c->hydro.count, c->grav.gcount) entries,
 ...
 * @param gbuff A buffer with at least max(c->hydro.count, c->grav.gcount) entries,
...
void cell_split(struct cell *c, ptrdiff_t parts_offset, ptrdiff_t sparts_offset,
  ...
  if (&c->progeny[7]->hydro.parts[c->progeny[7]->hydro.count] != &c->hydro.parts[count])
...
void cell_check_gpart_drift_point(struct cell *c, void *data) {
  ...
  if (c->grav.ti_old_gpart != ti_drift)
    error("Cell in an incorrect time-zone! c->grav.ti_old_gpart=%lld ti_drift=%lld",
...
void cell_check_multipole_drift_point(struct cell *c, void *data) {
  ...
  if (c->grav.ti_old_multipole != ti_drift)
    error(
        "Cell multipole in an incorrect time-zone! c->grav.ti_old_multipole=%lld "
...
void cell_activate_drift_part(struct cell *c, struct scheduler *s) {
  ...
  if (c == c->hydro.super) {
    ...
    for (struct cell *parent = c->parent;
         parent != NULL && !parent->hydro.do_sub_drift; parent = parent->parent) {
...
void cell_activate_sorts_up(struct cell *c, struct scheduler *s) {
  ...
  if (c == c->hydro.super) {
#ifdef SWIFT_DEBUG_CHECKS
    if (c->hydro.sorts == NULL) error("Trying to activate un-existing c->hydro.sorts");
#endif
    ...
    for (struct cell *parent = c->parent;
         parent != NULL && !parent->hydro.do_sub_sort; parent = parent->parent) {
...
int cell_unskip_stars_tasks(struct cell *c, struct scheduler *s) {
  ...
  for (struct link *l = c->stars_density; l != NULL; l = l->next) {
    ...
    if (t->type == task_type_pair || t->type == task_type_sub_pair) {
      ...
      /*   /\* If the local cell is active, send its ti_end values. *\/ */
      /*   if (cj_active) scheduler_activate_send(s, cj->mpi.send_ti, ci->nodeID);
       */
      ...
      /*   /\* If the local cell is active, send its ti_end values. *\/ */
      /*   if (ci_active) scheduler_activate_send(s, ci->mpi.send_ti, cj->nodeID);
       */
      /* } */
#endif
    }
