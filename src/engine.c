...
void engine_make_hierarchical_tasks_common(struct engine *e, struct cell *c) {
  ...
  if (c->super == c) {
    ...
    if (c->nodeID == e->nodeID) {
      ...
      c->hydro.end_force = scheduler_addtask(s, task_type_end_force,
                                       task_subtype_none, 0, 0, c, NULL);

      if (is_with_cooling) {

        c->hydro.cooling = scheduler_addtask(s, task_type_cooling, task_subtype_none,
                                       0, 0, c, NULL);
  ...
  if (c->grav.super == c) {
    ...
    if (c->nodeID == e->nodeID) {
      ...
      if (is_self_gravity) {
        ...
        scheduler_addunlock(s, c->grav.down, c->super->hydro.end_force);
...
void engine_addtasks_send_hydro(struct engine *e, struct cell *ci,
  ...
  if (l != NULL) {
    ...
    if (t_xv == NULL) {
      ...
      t_xv = scheduler_addtask(s, task_type_send, task_subtype_xv, ci->mpi.tag, 0,
                               ci, cj);
      t_rho = scheduler_addtask(s, task_type_send, task_subtype_rho, ci->mpi.tag, 0,
                                ci, cj);
#ifdef EXTRA_HYDRO_LOOP
      ...
      scheduler_addunlock(s, ci->hydro.super->extra_ghost, t_gradient);

      /* The send_rho task should unlock the super_hydro-cell's extra_ghost
       * task. */
      scheduler_addunlock(s, t_rho, ci->hydro.super->extra_ghost);
      ...
      scheduler_addunlock(s, t_rho, ci->super->hydro.end_force);
...
void engine_addtasks_send_gravity(struct engine *e, struct cell *ci,
  ...
  if (l != NULL) {
    ...
    if (t_grav == NULL) {
      ...
      t_grav = scheduler_addtask(s, task_type_send, task_subtype_gpart, ci->mpi.tag,
                                 0, ci, cj);
...
void engine_addtasks_send_timestep(struct engine *e, struct cell *ci,
  ...
  if (l != NULL) {
    ...
    if (t_ti == NULL) {
      ...
      t_ti = scheduler_addtask(s, task_type_send, task_subtype_tend, ci->mpi.tag, 0,
                               ci, cj);
...
void engine_addtasks_recv_hydro(struct engine *e, struct cell *c,
  ...
  if (t_xv == NULL && c->hydro.density != NULL) {
    ...
    /* Create the tasks. */
    t_xv = scheduler_addtask(s, task_type_recv, task_subtype_xv, c->mpi.tag, 0, c,
                             NULL);
    t_rho = scheduler_addtask(s, task_type_recv, task_subtype_rho, c->mpi.tag, 0, c,
                              NULL);
#ifdef EXTRA_HYDRO_LOOP
...
void engine_addtasks_recv_gravity(struct engine *e, struct cell *c,
  ...
  if (t_grav == NULL && c->grav.grav != NULL) {
    ...
    /* Create the tasks. */
    t_grav = scheduler_addtask(s, task_type_recv, task_subtype_gpart, c->mpi.tag, 0,
                               c, NULL);
  }
...
void engine_addtasks_recv_timestep(struct engine *e, struct cell *c,
  ...
  if (t_ti == NULL && (c->grav.grav != NULL || c->hydro.density != NULL)) {
    ...
    t_ti = scheduler_addtask(s, task_type_recv, task_subtype_tend, c->mpi.tag, 0, c,
                             NULL);
  }
  ...
  for (int pid = 0; pid < e->nr_proxies; pid++) {
    ...
    for (int k = 0; k < p->nr_cells_out; k++) {
      ...
      MPI_Isend(&buffer_send[this_send], num_elements, multipole_mpi_type,
                p->cells_in[0]->nodeID, p->cells_out[k]->mpi.tag, MPI_COMM_WORLD,
                &requests[this_request]);
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    if (t_type == task_type_sort) {
      for (struct cell *finger = t->ci->parent; finger != NULL;
        ...
        if (finger->hydro.sorts != NULL) scheduler_addunlock(sched, t, finger->hydro.sorts);
    }
...
static inline void engine_make_hydro_loops_dependencies(
  ...
  scheduler_addunlock(sched, gradient, c->hydro.super->extra_ghost);
  scheduler_addunlock(sched, c->hydro.super->extra_ghost, force);
}
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    /* Self-interaction? */
      ...
      engine_make_hydro_loops_dependencies(sched, t, t2, t3, t->ci,
                                           with_cooling);
      scheduler_addunlock(sched, t3, t->ci->super->hydro.end_force);
#else
      ...
      scheduler_addunlock(sched, t2, t->ci->super->hydro.end_force);
#endif
    }
    ...
    /* Otherwise, pair interaction? */
      ...
      if (t->cj->nodeID == nodeID) {
        ...
        if (t->ci->super != t->cj->super)
          scheduler_addunlock(sched, t3, t->cj->super->hydro.end_force);
      }
      ...
      if (t->cj->nodeID == nodeID) {
        if (t->ci->hydro.super != t->cj->hydro.super)
          engine_make_hydro_loops_dependencies(sched, t, t2, t->cj,
                                               with_cooling);
        if (t->ci->super != t->cj->super)
          scheduler_addunlock(sched, t2, t->cj->super->hydro.end_force);
      }
      ...
      if (t_subtype == task_subtype_density) {
        ...
        if (ci_nodeID != nodeID) {
          ...
          if (ci_active_hydro) {
            ...
            struct link *l = scheduler_activate_send(s, cj->hydro.send_xv, ci_nodeID);
          ...
          if (cj_active_hydro) {
            ...
            struct link *l = scheduler_activate_send(s, ci->hydro.send_xv, cj_nodeID);
...
/* Skip super-cells (Their values are already set) */
  ...
  for (int k = 0; k < 8; k++) {
    ...
    if (cp != NULL && (cp->hydro.count > 0 || cp->grav.gcount > 0 || cp->scount > 0)) {
    ...
    for (int i = 0; i < s->nr_cells; i++) {
      ...
      if (c->nodeID == engine_rank && c->hydro.count > 0) {
        ...
        for (int k = 1; k < c->hydro.count; k++) {
          if (c->hydro.parts[k].h > part_h_max) part_h_max = c->hydro.parts[k].h;
        }
