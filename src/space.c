...
static void rec_map_parts_xparts(struct cell *c,
  ...
  /* No progeny? */
  if (!c->split)
    for (int k = 0; k < c->hydro.count; k++) fun(&c->hydro.parts[k], &c->hydro.xparts[k], c);
...
void space_split_recursive(struct space *s, struct cell *c,
  ...
  if (s->nr_parts > 0)
    c->owner =
        ((c->hydro.parts - s->parts) % s->nr_parts) * s->nr_queues / s->nr_parts;
  else if (s->nr_sparts > 0)
    c->owner =
        ((c->sparts - s->sparts) % s->nr_sparts) * s->nr_queues / s->nr_sparts;
  else if (s->nr_gparts > 0)
    c->owner =
        ((c->grav.gparts - s->gparts) % s->nr_gparts) * s->nr_queues / s->nr_gparts;
  else
...
void space_getcells(struct space *s, int nr_cells, struct cell **cells) {
  ...
  for (int j = 0; j < nr_cells; j++) {
    ...
    cells[j]->nodeID = -1;
    if (lock_init(&cells[j]->hydro.lock) != 0 || lock_init(&cells[j]->grav.glock) != 0 ||
        lock_init(&cells[j]->grav.mlock) != 0 || lock_init(&cells[j]->slock) != 0)
