...
void engine_unskip(struct engine *e) {
  ...
  const int cells_per_top = 1 << (3 * level);
  const int total_size = multiplier * num_active_cells
    * cells_per_top * sizeof(struct cell*);
  local_active_cells = (struct cell **) malloc(total_size);
