  ...
  for (int k = 0; k < s->nr_cells; k++) {
    if (s->cells_top[k].nodeID == s->e->nodeID) {
      if (s->cells_top[k].hydro.h_max > part_h_max) {
        message("cell %d is inconsistent (%f > %f)", k, s->cells_top[k].hydro.h_max,
                part_h_max);
      }
    }
  }
...
int checkCellhdxmax(const struct cell *c, int *depth) {
  ...
  *depth = *depth + 1;
  ...
  if (c->hydro.h_max != h_max) {
    message("%d Inconsistent h_max: cell %f != parts %f", *depth, c->hydro.h_max,
            h_max);
    message("location: %f %f %f", c->loc[0], c->loc[1], c->loc[2]);
    result = 0;
  }
  if (c->hydro.dx_max != dx_max) {
    message("%d Inconsistent dx_max: %f != %f", *depth, c->hydro.dx_max, dx_max);
    message("location: %f %f %f", c->loc[0], c->loc[1], c->loc[2]);
    result = 0;
  }
  ...
  if (c->hydro.count > 0 || c->grav.gcount > 0 || c->scount > 0) {
    ...
    if (c->nodeID == e->nodeID &&
      ...
      fprintf(file,
              ...
              c->width[2], e->step, c->hydro.count, c->grav.gcount, c->scount, pactcount,
              c->depth, ntasks, c->hydro.ti_end_min,
