...
void runner_do_sort(struct runner *r, struct cell *c, int flags, int cleanup,
  ...
  for (struct cell *finger = c->parent; finger != NULL;
    ...
    if (finger->hydro.sorted & ~c->hydro.sorted) error("Inconsistent sort flags (upward).");
  }

  /* Update the sort timer which represents the last time the sorts
     were re-set. */
  if (c->hydro.sorted == 0) c->ti_sort = r->e->ti_current;
#endif
  ...
  if (c->split) {
    ...
    for (int k = 0; k < 8; k++) {
      if (c->progeny[k] != NULL && c->progeny[k]->hydro.count > 0) {
        ...
        dx_max_sort_old = max(dx_max_sort_old, c->progeny[k]->hydro.dx_max_sort_old);
      }
    }
  ...
  for (struct cell *finger = c->parent; finger != NULL;
       finger = finger->parent) {
    if (finger->hydro.sorted & ~c->hydro.sorted) error("Inconsistent sort flags.");
  }
#endif
