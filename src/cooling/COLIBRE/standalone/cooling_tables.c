...
void read_cooling_header(struct cooling_function_data *cooling) {
  ...
  if (posix_memalign((void **)&cooling->LogAbundances, SWIFT_STRUCT_ALIGNMENT,
                     colibre_cooling_N_metallicity *
                         colibre_cooling_N_abundances * sizeof(float)) != 0)
...
/**
 *  @brief converts metal masses to abundance ratios relative to solar abundance
 * ratios for their metallicity and returns the metallicity in log10 relative to
 * solar
...
float abundance_ratio_to_solar(
    struct particle_data *part, struct cooling_function_data *cooling,
...
void allocate_part_data(struct particle_data *part) {
  ...
  if (posix_memalign((void **)&part->metal_mass_fraction,
                     SWIFT_STRUCT_ALIGNMENT,
                     colibre_cooling_N_elementtypes * sizeof(float)) != 0)
