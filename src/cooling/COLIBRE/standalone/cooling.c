 ...
 */
void cooling_init_backend(struct cooling_function_data *cooling,
                          struct particle_data *part) {

  /* Setting the path for the hdf5 file */
  strcpy(cooling->cooling_table_path,
         "/cosma7/data/dp004/dc-ploe1/CoolingTables/2019_04/"
  ...
  double radiation_constant =
      4. * const_stefan_boltzmann_cgs / const_speed_light_c_cgs;
  cooling->Compton_constant_cgs =
      4. * const_boltzmann_k_cgs / const_electron_mass_cgs /
      const_speed_light_c_cgs * const_thomson_cross_section_cgs *
      radiation_constant;

  /* Reading in the table bins and abundances */
  read_cooling_header(cooling);

  /* Allocating the memory and reading in the multi-dimensional datasets for the
   * cooling rates et al. */
