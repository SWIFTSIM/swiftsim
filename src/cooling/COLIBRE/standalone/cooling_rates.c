...
double colibre_cooling_rate(
  ...
  Compton_cooling_rate = cooling->Compton_constant_cgs * electron_fraction /
                         n_H_cgs *
                         pow(const_T_CMB_0_cgs * (1. + redshift), 4.) *
                         (temp - const_T_CMB_0_cgs * (1. + redshift));

  printf("%.4f\t%.4f\t%.4e\t%.4e\t%.4e\t%.4e\n", log_u_cgs, logtemp,
         cooling_rate, heating_rate, Compton_cooling_rate, electron_fraction);
