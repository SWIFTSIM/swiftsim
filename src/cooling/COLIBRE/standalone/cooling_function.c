 ...
 * a given redshift, hydrogen number density (n_H_cgs), and element abundance
 * ratios (set as particle metal masses, part->metal_mass)
 *
 * @param cooling Cooling data structure
 * @param part Particle data structure
 */

void cooling_function(struct cooling_function_data *cooling,
  ...
  printf(
      ...
      "------\n");
  printf(
      "Abundance ratios (n_x / n_H) / (n_x / n_H)_table for logZZsol = %.2f \n",
      logZZsol);
  for (i = 0; i < colibre_cooling_N_elementtypes; i++) {
    printf("%i\t%.8f\n", i, abundance_ratio[i]);
  }
  ...
  printf(
      "log U\tlog T\tCooling rate\tHeating rate\tCompton cooling\tn_e/n_H \n");

  /* get indices for redshift, metallicity, and density */
  get_index_1d(cooling->Redshifts, colibre_cooling_N_redshifts, redshift,
               &red_index, &d_red);
  ...
  do {
    ...
    cooling_rate = colibre_cooling_rate(
        log_u_cgs, redshift, n_H_cgs, pow(10., logZZsol), abundance_ratio,
