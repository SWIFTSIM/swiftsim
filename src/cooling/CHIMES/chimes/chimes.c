...
void set_equilibrium_abundances_from_tables(struct UserData data)
{
  ...
  for (i = 0; i < data.myGlobalVars->totalNumberOfSpecies; i++) 
    data.myGasVars->abundances[i] = pow(10.0, chimes_interpol_3d(chimes_table_eqm_abundances.Abundances[i], T_index, nH_index, Z_index, dT, dnH, dZ)) * data.species[i].element_abundance; 
...
void chimes_network(struct gasVariables *myGasVars, struct globalVariables *myGlobalVars)
{
  ...
  ChimesFloat old_energy, new_energy, cool_rate, relative_change, this_absolute_tolerance; 
  ...
  if (max_relative_change < myGlobalVars->explicitTolerance) 
      ...
      else
	{
	  ...
	  NV_Ith_S(abstol_vector, data.network_size) = (realtype) myGlobalVars->thermalAbsoluteTolerance;
	}
