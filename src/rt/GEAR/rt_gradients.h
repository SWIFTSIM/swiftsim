 ...
 */
__attribute__((always_inline)) INLINE static void rt_gradients_update_part(struct part* restrict p, int g, float dE[3], float dFx[3], float dFy[3], float dFz[3]) {
...
__attribute__((always_inline)) INLINE static void rt_gradients_collect(
  ...
  kernel_deval(qi, &wi, &wi_dx); /* factor 1/omega for psi is swallowed in matrix */

  /* Compute kernel of pj */
  float wj, wj_dx;
  const float hj_inv = 1.0f / hj;
  const float qj = r * hj_inv;
  kernel_deval(qj, &wj, &wj_dx); /* factor 1/omega for psi is swallowed in matrix */

  /* Compute psi tilde */
  float psii_tilde[3];
  if (hydro_part_geometry_well_behaved(pi)) {
    psii_tilde[0] = wi * (Bi[0][0] * dx[0] + Bi[0][1] * dx[1] + Bi[0][2] * dx[2]);
    psii_tilde[1] = wi * (Bi[1][0] * dx[0] + Bi[1][1] * dx[1] + Bi[1][2] * dx[2]);
    psii_tilde[2] = wi * (Bi[2][0] * dx[0] + Bi[2][1] * dx[1] + Bi[2][2] * dx[2]);
  } else {
  ...
  if (hydro_part_geometry_well_behaved(pj)) {
    psij_tilde[0] = wi * (Bj[0][0] * dx[0] + Bj[0][1] * dx[1] + Bj[0][2] * dx[2]);
    psij_tilde[1] = wi * (Bj[1][0] * dx[0] + Bj[1][1] * dx[1] + Bj[1][2] * dx[2]);
    psij_tilde[2] = wi * (Bj[2][0] * dx[0] + Bj[2][1] * dx[1] + Bj[2][2] * dx[2]);
  } else {
  ...
  for (int g = 0; g < RT_NGROUPS; g++) {
    ...
    const float Qi[4] = {rtdi->density[g].energy, rtdi->density[g].flux[0], rtdi->density[g].flux[1], rtdi->density[g].flux[2]};
    const float Qj[4] = {rtdj->density[g].energy, rtdj->density[g].flux[0], rtdj->density[g].flux[1], rtdj->density[g].flux[2]};
    const float dQ[4] = {Qi[0] - Qj[0], Qi[1] - Qj[1], Qi[2] - Qj[2], Qi[3] - Qj[3]};
