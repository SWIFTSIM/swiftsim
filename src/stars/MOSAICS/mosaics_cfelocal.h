...
/* Mach number as a function of surface density, angular velocity and Toomre Q, from Krumholz & McKee (2005) */
INLINE static double f_mach(double sigmaloc, double csloc) 
{ 
    return sigmaloc/csloc; //Mach number
}

/* dispersion of overdensity PDF as a function of  Mach number and magnetic pressure ratio, based on e.g. Padoan & Nordlund (2011) */
INLINE static double f_sigrho(double mach, double beta0) 
{
    const double b=0.5; //constant
    return sqrt(log(1.+3.*b*b*mach*mach*beta0/(beta0+1.))); //dispersion
}

/* critical overdensity for SF in the KM05 sSFR_ff as a function of Mach number and GMC virial ratio, from Krumholz & McKee (2005) */
INLINE static double f_xcrit(double qvir, double mach) 
{
    const double phix=1.12; //constant
    return M_PI*M_PI*phix*phix/15.*qvir*mach*mach; //critical overdensity
}

/* specific star formation rate per free-fall time (sSFR_ff) for Elmegreen (2002, sflaw=0) or Krumholz & McKee (2005, sflaw=1) */
INLINE static double f_sfrff(double qvir, double mach, double beta0, double ecore, int sflaw) 
{
    ...
    if (sflaw == 1) 
        f=.5*ecore/phit*(1.+erf((-2.*log(xcrit)+sigrho*sigrho)/(2.*M_SQRT2*sigrho))); //specific star formation rate per free-fall time
    else
        f=0.012; //specific star formation rate per free-fall time
    return f;
}

/* overdensity PDF of the ISM as a function of overdensity x and its logarithmic mean and dispersion */
INLINE static double f_dpdx(double x, double mulnx, double sig) 
{
    double dif=log(x)-mulnx; //dummy
    return 1./(sqrt(2.*M_PI*sig*sig)*x)*exp(-.5*dif*dif/(sig*sig)); //overdensity PDF
}

/* rough relation between gas surface density and volume density and velocity dispersion, assuming an equilibrium disc */
INLINE static double f_surfg(double rholoc, double sigmaloc) 
{
    const double G=6.67e-11; //gravitational constant
    const double phiP=3.; //constant
    return sqrt(2.*rholoc*sigmaloc*sigmaloc/(M_PI*G*phiP)); //gas surface density
}

/* naturally bound fraction of star formation */
INLINE static double f_fstar (double rholoc, double sigmaloc, double csloc, double x, 
    double ecore, double beta0, double qvir, double tsn, double tview, 
    double surfGMC, int sflaw, int radfb)   
{
    double surfg,surffb,surffb3,mach,sfrff,rhog,tff,efb,einc,efbrad,fstar;
    ...
    if (radfb==0. || radfb==2)
        efb=0.5*sfrff*tsn/tff*(1.+sqrt(1.+4.*tff*sigmaloc*sigmaloc/(phifb*sfrff*tsn*tsn*x))); //  SN feedback
    else
        efb=1.;
    einc=sfrff*tview/tff; //star formation is incomplete/still ongoing
    if (radfb > 0.)
        efbrad=2.*sigSB/(phitrap*kappa0*kappa0*psi*surffb3)*(sqrt(1.+2.*M_PI*c*G*phitrap*kappa0*kappa0*surffb*surffb3/(1.*sigSB))-1.); //radiative feedback
    else
        efbrad=1.;
...
INLINE static double f_integrate(double xsurv, double mulnx, double sig, double rholoc, 
    double sigmaloc, double csloc, double ecore, double beta0, double qvir, 
    double tsn, double tview, double surfGMC, int cce, int sflaw, int radfb) 
{
    double xmin1,xmin2,xmax,xarr[1000],f1,dx,xg,fstar,bound,integral,dpdx,f2,frac;
    int nx,ix;
    ...
    if (cce > 0 && xsurv < xmin1) xmin1=xsurv; //if calculating the cruel cradle effect and critical overdensity below minimum, then adjust

    if (cce> 0 && xsurv > xmax) xmax=xsurv; //if calculating the cruel cradle effect and critical overdensity below maximum, then adjust
    ...
    for (ix=0; ix<nx; ix++) { //denominator integral
        ...
        if (cce == 2) bound = 1.; //if calculating the cruel cradle effect with respect to all SF, the denominator should contain all SF
    ...
    if (cce > 0) //if calculating the cruel cradle effect set minimum overdensity to critical overdensity
        xmin2 = xsurv;
    else 
        xmin2 = xmin1;
    ...
    for (ix=0; ix<nx; ix++) { //numerator integral
        ...
        fstar = f_fstar(rholoc,sigmaloc,csloc,xg,ecore,beta0,qvir,tsn,tview,surfGMC,sflaw,radfb); //local SFE
        bound=fstar/ecore; //local bound fraction
        if (cce == 2) bound=1. ; //if calculating the cruel cradle effect with respect to all SF, the numerator should contain all SF
        integral = bound*fstar*xarr[ix]; //integral part 1
        dpdx = f_dpdx(xg,mulnx,sig); //overdensity PDF, i.e. integral part 2
...
/* ratio of encounter timescale to energy dissipation timescale as a function of cloud virial ratio and overdensity */
INLINE static double f_phit(double qvir, double x) 
{
    return 3.1*sqrt((qvir/1.3)*(x/1.e4)); //ratio of encounter timescale to energy dissipation timescale
}
...
/* critical overdensity to remain bound despite the cruel cradle effect (also see Kruijssen et al. 2011) */
INLINE static double f_xcce(double sigmaloc, double surfGMC, double qvir, double tview) 
{
    double xmin,xmax,xfit,accuracy,xfit0,xarr[101],xarr2[101],phiad,diff,diffx;
    ...
    const double f = 0.7; //fraction of injected energy that is used for unbinding the region
    ...
    while (fabs(log10(xfit/xfit0)) > accuracy && niter < itermax) { //while iteration     does not give desired convergence, do
        xfit0 = xfit; //previously fitted x
        for (ix=0; ix<nx; ix++) { //for all x
            xarr[ix] = pow(10.,(ix-1)/(nx-1.)*log10(xmax/xmin)+log10(xmin)); //x array
            phiad = f_phiad(qvir,xarr[ix]); //adiabatic correction, see above
            xarr2[ix] = 87.5*sqrt(M_PI)*f*g_close*G*phish*surfGMC*phiad*tview/sigmaloc; //right-hand side of equation
        }
        diff = 1.e30;
        for (ix=0; ix<nx; ix++) { //for all x
...
/** TODO hide this within a DEBUG def
    if (niter == itermax){ //if we stopped due to reaching maximum number of iterations, then stop
        printf("f_xcce: no convergence, increase nx or itermax in the f_xcce subroutine");
        endrun(932);
    }
*/
    return xfit;
...
INLINE static double feedback_timescale(double rholoc, double sigmaloc, double csloc,
    const struct stars_props* stars_properties) 
{ 
    ...
    // SN feedback
    return 0.5*tsn*(1.+sqrt(1.+4.*tff*sigmaloc*sigmaloc/(phifb*sfrff*tsn*tsn*x)));
}
