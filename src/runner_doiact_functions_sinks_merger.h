...
void DOSELF1_SINKS(struct runner *r, struct cell *c, int timer) {
  ...
  for (int sid = 0; sid < scount; sid++) {
    ...
    for (int sjd = 0; sjd < scount; pjd++) {

      /* Get a pointer to the jth particle. */
      struct sink *restrict sj = &sinks[pjd];
...
void DO_NONSYM_PAIR1_SINKS_NAIVE(struct runner *r, struct cell *restrict ci,
  ...
  for (int sid = 0; sid < scount_i; sid++) {
    ...
    for (int sjd = 0; sjd < scount_j; sjd++) {
      ...
      /* Get a pointer to the jth particle. */
      struct sink *restrict sj = &sinks_j[pjd];
      const float rj = sj->r_cut;
  ...
  if (do_cj_sinks) {
    ...
    for (int j = 0; j < scount_j; j++) {
      ...
      for (int i = 0; i < scount_i; i++) {
        ...
        /* Skip inhibited particles. */
        if (sink_is_inhibited(pi, e)) continue;
        ...
        if (r2 < rj2) {
          ...
          IACT_SINK(r2, dx, rj, ri, spj, spi, a, H);
        }
      } /* loop over the sinks in ci. */
    }   /* loop over the sinks in cj. */
  }     /* Cell cj is active */
