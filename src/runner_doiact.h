...
void DOPAIR1(struct runner *r, struct cell *ci, struct cell *cj, const int sid,
  ...
  const float shift_threshold_x =
      2. * ci->width[0] + 2. * max(ci->hydro.dx_max, cj->dx_max);
  const float shift_threshold_y =
      2. * ci->width[1] + 2. * max(ci->hydro.dx_max, cj->dx_max);
  const float shift_threshold_z =
      2. * ci->width[2] + 2. * max(ci->hydro.dx_max, cj->dx_max);
...
void DOPAIR1_BRANCH(struct runner *r, struct cell *ci, struct cell *cj) {
  ...
  for (int pid = 0; pid < ci->hydro.count; pid++) {
    ...
    if (fabsf(d - sort_i[pid].d) - ci->hydro.dx_max_sort >
        ...
        fabsf(d - sort_i[pid].d) - ci->hydro.dx_max_sort > ci->width[0] * 1.0e-10)
      error(
  ...
  for (int pjd = 0; pjd < cj->hydro.count; pjd++) {
    ...
    if ((fabsf(d - sort_j[pjd].d) - cj->hydro.dx_max_sort) >
        ...
        (fabsf(d - sort_j[pjd].d) - cj->hydro.dx_max_sort) > cj->width[0] * 1.0e-10)
      error(
    ...
    for (int k = 0; k < 8; k++) {
      if (ci->progeny[k] != NULL) {
        if (&parts[ind[0]] >= &ci->progeny[k]->hydro.parts[0] &&
            &parts[ind[0]] < &ci->progeny[k]->hydro.parts[ci->progeny[k]->hydro.count]) {
          sub = ci->progeny[k];
          break;
        }
      }
