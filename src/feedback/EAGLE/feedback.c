...
void print_array_double(FILE *file, double *array, int array_size){
  for (int i = 0; i < array_size; i++) fprintf(file,"%.5e\n",array[i]);
}

void print_array_float(FILE *file, float *array, int array_size){
  for (int i = 0; i < array_size; i++) fprintf(file,"%.5e\n",array[i]);
}

void print_array_string(FILE *file, char **array, int array_size){
  for (int i = 0; i < array_size; i++) fprintf(file,"%s\n",array[i]);
}

void print_feedback_struct(const struct feedback_props *fp){
  FILE *feedback_output;
  feedback_output = fopen("feedback_props_out.log","w");

  fprintf(feedback_output,"switch constants--------------------\n");
  ...
  fprintf(feedback_output,"yield tables AGB--------------------\n");
  print_array_double(feedback_output,fp->yield_AGB.mass,eagle_feedback_AGB_N_masses);
  print_array_double(feedback_output,fp->yield_AGB.metallicity,eagle_feedback_AGB_N_metals);
  print_array_double(feedback_output,fp->yield_AGB.yield_IMF_resampled,eagle_feedback_AGB_N_metals*eagle_feedback_N_imf_bins*chemistry_element_count);
  print_array_double(feedback_output,fp->yield_AGB.yield,eagle_feedback_AGB_N_metals * eagle_feedback_AGB_N_masses * eagle_feedback_AGB_N_elements );
  print_array_double(feedback_output,fp->yield_AGB.ejecta_IMF_resampled,eagle_feedback_AGB_N_metals * eagle_feedback_N_imf_bins);
  print_array_double(feedback_output,fp->yield_AGB.ejecta, eagle_feedback_AGB_N_metals * eagle_feedback_AGB_N_masses);
  print_array_double(feedback_output,fp->yield_AGB.total_metals_IMF_resampled,eagle_feedback_AGB_N_metals * eagle_feedback_N_imf_bins);
  print_array_double(feedback_output,fp->yield_AGB.total_metals,eagle_feedback_AGB_N_metals * eagle_feedback_AGB_N_masses);
  
  fprintf(feedback_output,"yield tables SNIa--------------------\n");
  print_array_double(feedback_output,fp->yield_SNII.mass,eagle_feedback_SNII_N_masses);
  print_array_double(feedback_output,fp->yield_SNII.metallicity,eagle_feedback_SNII_N_metals);
  print_array_double(feedback_output,fp->yield_SNII.yield_IMF_resampled,eagle_feedback_SNII_N_metals*eagle_feedback_N_imf_bins*chemistry_element_count);
  print_array_double(feedback_output,fp->yield_SNII.yield,eagle_feedback_SNII_N_metals * eagle_feedback_SNII_N_masses * eagle_feedback_SNII_N_elements );
  print_array_double(feedback_output,fp->yield_SNII.ejecta_IMF_resampled,eagle_feedback_SNII_N_metals * eagle_feedback_N_imf_bins);
  print_array_double(feedback_output,fp->yield_SNII.ejecta, eagle_feedback_SNII_N_metals * eagle_feedback_SNII_N_masses);
  print_array_double(feedback_output,fp->yield_SNII.total_metals_IMF_resampled,eagle_feedback_SNII_N_metals * eagle_feedback_N_imf_bins);
  print_array_double(feedback_output,fp->yield_SNII.total_metals,eagle_feedback_SNII_N_metals * eagle_feedback_SNII_N_masses);

  fprintf(feedback_output,"yield tables SNIa--------------------\n");
  print_array_double(feedback_output,fp->yield_SNIa_IMF_resampled, eagle_feedback_SNIa_N_elements);
  fprintf(feedback_output,"%.5e\n",fp->yield_SNIa_total_metals_IMF_resampled);
  print_array_double(feedback_output, fp->yields_SNIa, eagle_feedback_SNIa_N_elements);

  fprintf(feedback_output,"element names--------------------\n");
  print_array_string(feedback_output, fp->SNIa_element_names, eagle_feedback_SNIa_N_elements);
  print_array_string(feedback_output, fp->SNII_element_names, eagle_feedback_SNII_N_elements);
  print_array_string(feedback_output, fp->AGB_element_names, eagle_feedback_AGB_N_elements);

  fprintf(feedback_output,"mass bins--------------------\n");
  print_array_double(feedback_output, fp->yield_mass_bins, eagle_feedback_N_imf_bins);

  fprintf(feedback_output,"yield table path--------------------\n");
  fprintf(feedback_output,"%s\n",fp->yield_table_path);
  
  fprintf(feedback_output,"lifetime table--------------------\n");
  print_array_double(feedback_output, fp->lifetimes.mass, eagle_feedback_lifetime_N_masses);
  print_array_double(feedback_output, fp->lifetimes.metallicity, eagle_feedback_lifetime_N_metals);
  for (int i = 0; i < eagle_feedback_lifetime_N_metals; i++) 
    print_array_double(feedback_output, fp->lifetimes.dyingtime[i], eagle_feedback_lifetime_N_masses);

  fprintf(feedback_output,"constants--------------------\n");
  for (int i = 0; i < chemistry_element_count; i++) 
    fprintf(feedback_output, "%.5e\n", fp->SNII_yield_factor[i]);
  fprintf(feedback_output, "%.5e\n", fp->SNIa_efficiency);
  fprintf(feedback_output, "%.5e\n", fp->SNIa_timescale_Gyr);
  fprintf(feedback_output, "%.5e\n", fp->SNIa_timescale_Gyr_inv);
  fprintf(feedback_output, "%.5e\n", fp->log10_SNIa_max_mass_msun);
  fprintf(feedback_output, "%.5e\n", fp->E_SNIa_cgs);
  fprintf(feedback_output, "%.5e\n", fp->E_SNIa);
  fprintf(feedback_output, "%.5e\n", fp->AGB_ejecta_specific_kinetic_energy);
  fprintf(feedback_output, "%.5e\n", fp->mass_to_solar_mass);
  fprintf(feedback_output, "%.5e\n", fp->solar_mass_to_mass);
  fprintf(feedback_output, "%.5e\n", fp->rho_to_n_cgs);
  fprintf(feedback_output, "%.5e\n", fp->temp_to_u_factor);

  fprintf(feedback_output,"imf--------------------\n");
  print_array_float(feedback_output, fp->imf, eagle_feedback_N_imf_bins);

  fprintf(feedback_output,"imf mass bin--------------------\n");
  print_array_float(feedback_output, fp->imf_mass_bin, eagle_feedback_N_imf_bins);

  fprintf(feedback_output,"imf mass bin log10--------------------\n");
  print_array_float(feedback_output, fp->imf_mass_bin_log10, eagle_feedback_N_imf_bins);

  fprintf(feedback_output,"imf mass bounds-------------------\n");
  fprintf(feedback_output, "%.5e\n", fp->imf_min_mass_msun);
  fprintf(feedback_output, "%.5e\n", fp->imf_max_mass_msun);
  fprintf(feedback_output, "%.5e\n", fp->log10_imf_min_mass_msun);
  fprintf(feedback_output, "%.5e\n", fp->log10_imf_max_mass_msun);

  fprintf(feedback_output,"other constants-------------------\n");
  fprintf(feedback_output, "%.5e\n", fp->log10_SNII_min_mass_msun);
