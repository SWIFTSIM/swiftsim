 ...
 * points is computed
 */
__attribute__((always_inline)) INLINE static float compute_arclength(
  ...
  const float arc_length =
      2.f * r_sphere *
      asin(sqrt(sin(delta_theta / 2.0) * sin(delta_theta / 2.0) +
                cos(theta_1) * cos(theta_2) * sin(delta_phi / 2.0) *
                    sin(delta_phi / 2.0)));
...
__attribute__((always_inline)) INLINE static float find_min_arclength(
  ...
  arclengths requires \theta \in [-\pi/2, \pi/2] but we have \theta \in [0,\pi]
*/
  const float new_arclength = compute_arclength(
