...
INLINE static void compute_stellar_momentum(struct spart* sp, 
  ...
  /* define a few conversion factors that are needed later on */
  const double conv_fact   = us->UnitMass_in_cgs * (us->UnitLength_in_cgs / 
						    us->UnitTime_in_cgs);
...
void feedback_props_init(struct feedback_props* fp,
  ...
  fp->f_E_min =
      parser_get_param_double(params, "COLIBREFeedback:SNII_energy_fraction_min");
  fp->f_E_max =
      parser_get_param_double(params, "COLIBREFeedback:SNII_energy_fraction_max");
  fp->Z_0 =
      parser_get_param_double(params, "COLIBREFeedback:SNII_energy_fraction_Z_0");
  fp->n_0_cgs = parser_get_param_double(
      params, "COLIBREFeedback:SNII_energy_fraction_n_0_H_p_cm3");
  fp->n_n =
      parser_get_param_double(params, "COLIBREFeedback:SNII_energy_fraction_n_n");
  fp->n_Z =
