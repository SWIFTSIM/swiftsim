...
/**
 * @brief Calculates the arc length on a sphere of radius r_sphere between
 ...
 * arc length with the currently existing arc length for the considered ray
 ...
 * Returns the new arc length if it is smaller than the older one or zero
 * otherwise
 *
 ...
 * @param r_sphere Radius of the sphere on which the arc length between the two
 * points is computed
 * @param current_archlength Current minimal value of the ray's arc length
 */
__attribute__((always_inline)) INLINE static float find_min_arclength(
   ...
   * arc lengths requires \theta \in [-\pi/2, \pi/2] but we have \theta \in
   * [0, \pi]  */
  const float new_arclength = arclength(
      theta_ray - M_PI_2, phi_ray, theta_part - M_PI_2, phi_part, r_sphere);

  /* Compare the arc length that's just been computed with what the ray already
   * has. If the new one is smaller then return the computed one. */
  if (new_arclength < current_arclength) {
    return new_arclength;
  }
  /* If the new arc length is larger than the current one return zero */
  else {
    return 0.f;
  }
}
