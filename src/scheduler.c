    ...
    /* Pair interaction? */
      ...
      if (cell_can_split_pair_hydro_task(ci) &&
        ...
        if (scheduler_dosub && /* Use division to avoid integer overflow. */
            ci->hydro.count * sid_scale[sid] < space_subsize_pair_hydro / cj->hydro.count &&
            !sort_is_corner(sid)) {
        ...
        // message( "force splitting pair with %i and %i parts." , ci->hydro.count ,
        // cj->hydro.count );
      ...
      case task_type_recv:
        ...
        if (t->subtype == task_subtype_tend) {
          t->buff = (struct pcell_step *)malloc(sizeof(struct pcell_step) *
                                                t->ci->mpi.pcell_size);
          err = MPI_Irecv(
              t->buff, t->ci->mpi.pcell_size * sizeof(struct pcell_step), MPI_BYTE,
          ...
          err = MPI_Irecv(t->ci->grav.gparts, t->ci->grav.gcount, gpart_mpi_type,
                          t->ci->nodeID, t->flags, subtaskMPI_comms[t->subtype],
                          &t->req);
      ...
      case task_type_send:
        ...
        if (t->subtype == task_subtype_tend) {
          ...
          if ((t->ci->hydro.count * sizeof(struct part)) > s->mpi_message_limit)
            err = MPI_Isend(t->ci->hydro.parts, t->ci->hydro.count, part_mpi_type,
                            t->cj->nodeID, t->flags,
                            subtaskMPI_comms[t->subtype], &t->req);
          else
            err = MPI_Issend(t->ci->hydro.parts, t->ci->hydro.count, part_mpi_type,
                             t->cj->nodeID, t->flags,
                             subtaskMPI_comms[t->subtype], &t->req);
          ...
          if ((t->ci->grav.gcount * sizeof(struct gpart)) > s->mpi_message_limit)
            err = MPI_Isend(t->ci->grav.gparts, t->ci->grav.gcount, gpart_mpi_type,
                            t->cj->nodeID, t->flags,
                            subtaskMPI_comms[t->subtype], &t->req);
          else
            err = MPI_Issend(t->ci->grav.gparts, t->ci->grav.gcount, gpart_mpi_type,
                             t->cj->nodeID, t->flags,
                             subtaskMPI_comms[t->subtype], &t->req);
        } else if (t->subtype == task_subtype_spart) {
