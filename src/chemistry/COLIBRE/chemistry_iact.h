...
__attribute__((always_inline)) INLINE static void runner_iact_diffusion(
  ...
  if (chj->diffusion_coefficient > 0 || chi->diffusion_coefficient > 0) {
    ...
    const float Eu_mi = mi * chi->metal_mass_fraction[chemistry_element_Eu];
    const float Eu_mj = mj * chj->metal_mass_fraction[chemistry_element_Eu];
    
    float chi_Eu_fraction_from_NSM = chi->mass_from_NSM / Eu_mi;
    float chj_Eu_fraction_from_NSM = chj->mass_from_NSM / Eu_mj;
      chi->dEu_mass_fraction_from_NSM += K_ij * (chi_Eu_fraction_NSM - chj_Eu_fraction_NSM) * dt;

      chj->dEu_mass_fraction_from_NSM += K_ji * (chj_Eu_fraction_NSM - chi_Eu_fraction_NSM) * dt;

    float chi_Eu_fraction_from_CEJSN = chi->mass_from_CEJSN / Eu_mi;
    float chj_Eu_fraction_from_CEJSN = chj->mass_from_CEJSN / Eu_mj;
      chi->dEu_mass_fraction_from_CEJSN += K_ij * (chi_Eu_fraction_CEJSN - chj_Eu_fraction_CEJSN) * dt;
      
      chj->dEu_mass_fraction_from_CEJSN += K_ji * (chj_Eu_fraction_CEJSN - chi_Eu_fraction_CEJSN) * dt;

    float chi_Eu_fraction_from_collapsar = chi->mass_from_collapsar / Eu_mi;
    float chj_Eu_fraction_from_collapsar = chj->mass_from_collapsar / Eu_mj;
      chi->dEu_mass_fraction_from_collapsar += K_ij * (chi_Eu_fraction_collapsar - chj_Eu_fraction_collapsar) * dt;
      
      chj->dEu_mass_fraction_from_collapsar += K_ji * (chj_Eu_fraction_collapsar - chi_Eu_fraction_collapsar) * dt;
