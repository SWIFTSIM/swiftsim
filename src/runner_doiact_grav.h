...
static INLINE void runner_do_grav_down(struct runner *r, struct cell *c,
  ...
  if (c->grav.ti_old_multipole != e->ti_current) error("c->multipole not drifted.");
    ...
    for (int k = 0; k < 8; ++k) {
      ...
      if (cp != NULL && cell_is_active_gravity(cp, e)) {
        ...
        if (c->grav.multipole->pot.interacted) {
          ...
          gravity_L2L(&shifted_tensor, &c->grav.multipole->pot, cp->grav.multipole->CoM,
                      c->grav.multipole->CoM);
      ...
      if (ci_active) {
        ...
        if (allow_mpole)
          runner_dopair_grav_pm_truncated(ci_cache, gcount_padded_i, CoM_j,
                                          multi_j, dim, r_s_inv, e, ci->grav.gparts,
                                          gcount_i, cj);
      }
      if (cj_active && symmetric) {
        ...
        if (allow_mpole)
          runner_dopair_grav_pm_truncated(cj_cache, gcount_padded_j, CoM_i,
                                          multi_i, dim, r_s_inv, e, cj->grav.gparts,
                                          gcount_j, ci);
      }
...
static INLINE void runner_doself_grav_pp(struct runner *r, struct cell *c) {
  ...
  if (!periodic) {
    ...
    /* Not periodic -> Can always use Newtonian potential */
    runner_doself_grav_pp_full(ci_cache, gcount, gcount_padded, e, c->grav.gparts);
...
static INLINE void runner_dopair_grav_mm_symmetric(struct runner *r,
  ...
  gravity_M2L_symmetric(&ci->grav.multipole->pot, &cj->grav.multipole->pot, multi_i,
                        multi_j, ci->grav.multipole->CoM, cj->grav.multipole->CoM, props,
  ...
  if (ci->split) {
    ...
    if (!periodic) {
      ...
      runner_dopair_grav_pm_truncated(ci_cache, gcount_padded_i, CoM_j, multi_j,
                                      dim, r_s_inv, e, ci->grav.gparts, gcount_i,
                                      cj);
    }
...
static INLINE void runner_dopair_recursive_grav(struct runner *r,
  ...
  if (cell_is_active_gravity(ci, e) && ci->grav.ti_old_multipole != e->ti_current)
    error("ci->grav.multipole not drifted.");
  if (cell_is_active_gravity(cj, e) && cj->grav.ti_old_multipole != e->ti_current)
    error("cj->grav.multipole not drifted.");
#endif
