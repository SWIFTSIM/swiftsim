...
def check_all_hydro_is_equal(snapdata):
    ...
    for compare in snapdata[1:]:
        ...
        sml_diff = np.abs(1. - ref.gas.h / compare.gas.h)
            ...
            if print_diffs:
                for i in range(npart):
                    if ref.gas.InjectionDone[i] != compare.gas.InjectionDone[i]:
                        print("-----", ref.gas.IDs[i], ref.gas.InjectionDone[i], compare.gas.InjectionDone[i])
        ...
        if fishy.any():
            print("- Comparing hydro", ref.snapnr, "->", compare.snapnr)
            account_for_sml_diff = np.count_nonzero(np.logical_and(fishy, sml_within_tolerance))
            print("--- Calls to iact gradient: count differ: {0:8d} / {1:8d}; After removing ones with acceptable h differences: {2:8d}".format(np.count_nonzero(fishy), npart, account_for_sml_diff))

            if account_for_sml_diff > 0:
                if print_diffs:
                    for i in range(npart):
                        if ref.gas.RTCallsIactGradient[i] != compare.gas.RTCallsIactGradient[i]:
                            print("-----", ref.gas.IDs[i], ref.gas.RTCallsIactGradient[i], 
                                    compare.gas.RTCallsIactGradient[i], 
                                    sml_within_tolerance[i], sml_diff[i])
        ...
        if fishy.any():
            print("- Comparing hydro", ref.snapnr, "->", compare.snapnr)
            account_for_sml_diff = np.count_nonzero(np.logical_and(fishy, sml_within_tolerance))
            print("--- Calls to iact transport: count differ: {0:8d} / {1:8d}; After removing ones with acceptable h differences: {2:8d}".format(np.count_nonzero(fishy), npart, account_for_sml_diff))

            if account_for_sml_diff > 0:
                if print_diffs:
                    for i in range(npart):
                        if ref.gas.RTCallsIactTransport[i] != compare.gas.RTCallsIactTransport[i]:
                            print("-----", ref.gas.IDs[i], ref.gas.RTCallsIactTransport[i], 
...
def main():
    ...
    snapdata, rundata = get_snap_data(prefix=file_prefix,  skip_snap_zero=skip_snap_zero, skip_last_snap=skip_last_snap)
