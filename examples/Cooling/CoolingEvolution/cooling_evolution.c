...
INLINE double dTdt_isochoric_ionized(double temp,  double redshift, double n_H, float ZZsol, 
  ...
  dT = ne * n_H * lambda_net / (3./2. * ntotal * const_boltzmann_k_cgs);
...
/* Choke on FP-exceptions */
  ...
  char cool_outputfile[35];
  sprintf(cool_outputfile, "cooling_output_lognH%.2f.dat", log_10_nh);
  char heat_outputfile[35];
  sprintf(heat_outputfile, "heating_output_lognH%.2f.dat", log_10_nh);
  char net_outputfile[35];
  sprintf(net_outputfile, "netcooling_output_lognH%.2f.dat", log_10_nh);

  // Calculate contributions from metals to cooling rate
  // open file
  FILE *output_file = fopen(cool_outputfile, "w");
  if (output_file == NULL) {
    error("Error opening output file!\n");
  }
  FILE *output_file_heat = fopen(heat_outputfile, "w");
  if (output_file_heat == NULL) {
    error("Error opening output file (heat)!\n");
  }
  FILE *output_file_net = fopen(net_outputfile, "w");
  if (output_file_net == NULL) {
  ...
  double k1, k2, k3, k4, temperature_new;
  ...
  do {
    ...
    do {
      ...
      k1 = hstep * dTdt_isochoric_ionized(temperature,
        cosmo.z, nh, pow(10., logZZsol), abundance_ratio, n_H_index,
        d_n_H, met_index, d_met, red_index, d_red, &cooling);

      k2 = hstep * dTdt_isochoric_ionized(temperature + 1./2. * k1,
        cosmo.z, nh, pow(10., logZZsol), abundance_ratio, n_H_index,
        d_n_H, met_index, d_met, red_index, d_red, &cooling);

      k3 = hstep * dTdt_isochoric_ionized(temperature + 1./2. * k2,
        cosmo.z, nh, pow(10., logZZsol), abundance_ratio, n_H_index,
        d_n_H, met_index, d_met, red_index, d_red, &cooling);

      k4 = hstep * dTdt_isochoric_ionized(temperature + k3,
        cosmo.z, nh, pow(10., logZZsol), abundance_ratio, n_H_index,
        d_n_H, met_index, d_met, red_index, d_red, &cooling);

      // lin
      temperature_new = pow(10., temperature) + 1./6. *  k1 + 1./3. * k2 + 1./3. * k3 + 1./6. * k4;
    } while (abs(1. - temperature_new / pow(10., temperature)) > rtol);

    time += hstep;

    printf("%.4e\t%.4e\t%.4e\t%.4e\n", time, hstep, temperature, temperature_new );

    // lin
    temperature = temperature_new;

  } while( temperature_new > 1.e4);
  fclose(output_file);
