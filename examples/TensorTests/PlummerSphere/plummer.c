...
int main(int argc, char *argv[])
{
  ...
  double sv = (1.02249e-6/1000.0) /sx *sqrt(pow(RADIUS*1000.0,3.)/(2.0*2.2489e-15*MASS)); // (kmskpy)*scl/sct //sqrt(1.0/sx);
  for (i=0;i<n;i++) {
    for (j=0;j<3;j++) {
      pos[3*i+j] -= cm[j];
      vel[3*i+j] -= cm[j+3];
  ...
  for (i=0;i<n;i++) {
    fprintf(file,"%d %14.7e %15.7e %15.7e %15.7e %15.7e %15.7e %15.7e\n", i, mass, pos[3*i], pos[3*i+1], pos[3*i+2],vel[3*i], vel[3*i+1], vel[3*i+2]);
  }
  fclose(file);

  double ff, tcr, rpl = RADIUS*1000.;
  ff = 32.0*M_PI*M_PI*sqrt(2.0)*(22.0*22.0*22.0)/(1.0+22.0*22.0)*rpl*rpl*rpl;
  ...
  for (i=0;i<n;i++) T += vel[3*i]*vel[3*i] + vel[3*i+1]*vel[3*i+1] + vel[3*i+2]*vel[3*i+2];
  T *= 0.5*mass;
  for (i=0;i<n;i++) {
    x = pos[3*i];
    y = pos[3*i+1];
