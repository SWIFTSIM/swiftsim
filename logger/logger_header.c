...
void header_read(struct header *h, struct logger_logfile *log) {
  ...
  for (int i = 0; i < hydro_logger_field_count; i++) {
    if (hydro_logger_local_to_global[i] == -1)
      error_python(
          "Field %s in hydro_logger_reader_populate_mask_data is not set",
  ...
  for (int i = 0; i < gravity_logger_field_count; i++) {
    if (gravity_logger_local_to_global[i] == -1)
      error_python(
          "Field %s in gravity_logger_reader_populate_mask_data is not set",
  ...
  for (int i = 0; i < stars_logger_field_count; i++) {
    if (stars_logger_local_to_global[i] == -1)
      error_python(
          "Field %s in stars_logger_reader_populate_mask_data is not set",
